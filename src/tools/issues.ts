...
export function registerIssueTools(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerTool("search_issues", {
    ...
    return content({ total_count: resp.data.total_count, items });
  });
  ...
  if (isGateEnabled("write", config)) {
    ...
    server.registerTool("add_issue_comment", {
      ...
      return content({ message: "Comment added", url: (resp.data as Record<string, unknown>).html_url });
    });
    ...
    server.registerTool("add_issue_labels", {
      ...
      return content({ issue_number: params.issue_number, labels: resp.data });
    });
    ...
    server.registerTool("remove_issue_label", {
      ...
      return content({ message: `Label ${params.label} removed from issue #${params.issue_number}` });
    });
    ...
    server.registerTool("lock_issue", {
      ...
      return content({ message: `Issue #${params.issue_number} has been locked` });
    });
  }
